...
CDelta::CDelta(unsigned int _iWin, unsigned int _iOrd) : ADataProcessor()
{
    ...
    for (unsigned int i = 0; i < m_iBf; i++) {
...
void CDelta::getData(CDataContainer &_pData)
{
    ...
    /// extend its length
    _pData.reserve((m_iOrd + 1) * m_iSize);
    _pData.size() = (m_iOrd + 1) * m_iSize;

    /// compute the coefficients
    for (i = (m_iOrd - 1) * m_iSize; i < m_iOrd * m_iSize; i++) {
        norm = 0;
        _pData[m_iSize + i] = 0.0;
        for (j = 1; j <= m_iWin; j++) {
            _pData[m_iSize + i] += j * ((*(m_pBuffer[m_iWin - j]))[i] - (*(m_pBuffer[m_iWin + j]))[i]);
            norm += j * j;
        }
