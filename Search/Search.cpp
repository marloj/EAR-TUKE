...
unsigned int CSearch::initialize(EAR_FST_Net *_pNet, CAcousticScorer *_pScorer, float _fWordInsPenalty)
{
    ...
    m_iEndState = 0;
    for (unsigned int i = 0; i < m_pNet->iSize; i++)
        if (m_pNet->pNet[i].iStart > m_iEndState)
            m_iEndState = m_pNet->pNet[i].iStart;
...
void CSearch::propagateEmpty(CToken *_token)
{
    ...
    if (!iSym && _token->pPrev)
        iSym = _token->pPrev->iSym;
    ...
    while (m_pNet->pNet[iPos].iStart == iStart) ///< transitions with the same starting state
    {
        ...
        if (m_pNet->pNet[iPos].iEnd == UNDEF_STATE) {
            iPos++;
            continue;
        }
        ...
        if (m_pNet->pNet[iPos].iIn == EPS_SYM) {
            ...
            if (_token->iSym)
                token = m_pTokens->add(_token);
            else
                token = m_pTokens->add(_token->pPrev);
            ...
            if (m_pNet->pNet[iPos].iOut && m_pNet->pNet[iPos].iOut != iSym) {
                ...
                token->addAuxScore((-1) * m_pNet->pNet[iPos].fWeight);
            }
...
void CSearch::insert(CToken *_token, unsigned int _iState)
{
    ...
    if (_iState >= m_iStates) {
        m_pTokens->ret(_token);
        return;
    }
...
void CSearch::getResults(CResults &_results)
{
    ...
    if (pToken == NULL || pToken->pPrev == NULL)
        return;
