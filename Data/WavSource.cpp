...
unsigned int CWavSource::load(char *_szFileName)
{
    ...
    if (!pf) {
        printf("CWavSource: Error opening input file %s\n", _szFileName);
        return EAR_FAIL;
    }
    ...
    while ((c = getc(pf)) != EOF) {
        if (c != 'f') continue;
        if ((c = getc(pf)) == EOF) return EAR_FAIL;
        if (c != 'm') continue;
        if ((c = getc(pf)) == EOF) return EAR_FAIL;
        if (c != 't') continue;
        if ((c = getc(pf)) == EOF) return EAR_FAIL;
        ...
        fread(&iT, 2, 1, pf);
        if (iT != 1) {
            printf("CWavSource: This Audio File is Compressed, Compression is not Supported\n");
            return EAR_FAIL;
        }
        fread(&iT, 2, 1, pf);
        if (iT != 1) {
            printf("CWavSource: This Audio has more than one channel, this is not supported\n");
            return EAR_FAIL;
        }
        ...
        m_iBytesPerSmp /= 8;
        if (m_iBytesPerSmp != 2 && m_iBytesPerSmp != 1 && m_iBytesPerSmp != 3) {
            printf("Not supported number of bits per sample %d\n", m_iBytesPerSmp * 8);
            return EAR_FAIL;
        }
        break;
    }
...
void CWavSource::getData(CDataContainer &_pData)
{
    ...
    if (iAvail > m_iReadLength) {
        iAvail = m_iReadLength;
    }
    iAvail /= m_iBytesPerSmp;
    _pData.reserve(iAvail);

    /// compute start pointers
    unsigned int i = 0;
    float *pDst = _pData.data();
    unsigned char *pSrc = m_psBuf + m_iRead;

    /// transform and copy data
    if (m_iBytesPerSmp == 1) {
        for (i = 0; i < iAvail; i++) {
            pDst[i] = ((float) pSrc[i] - 128);
        }
    }

    if (m_iBytesPerSmp == 2) {
        for (i = 0; i < iAvail; i++) {
            pDst[i] = ((float) ((short*) pSrc)[i]);
        }
    }
